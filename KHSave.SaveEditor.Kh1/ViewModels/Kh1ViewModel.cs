...
namespace KHSave.SaveEditor.Kh1.ViewModels
{
        ...
        IGetAbilities
    {
        ...
        public Visibility JournalVisible => Journal is null ? Visibility.Collapsed : Visibility.Visible;

        public void RefreshUi()
        {
            System = new SystemViewModel(Save, this);
            Inventory = new InventoryViewModel(Save);
            Journal = Save is SaveKh1.SaveFinalMix finalMixSave ? new JournalViewModel(finalMixSave) : null;
            Players = new PlayersViewModel(Save, this);

            OnPropertyChanged(nameof(System));
            OnPropertyChanged(nameof(Inventory));
            OnPropertyChanged(nameof(Journal));
            OnPropertyChanged(nameof(JournalVisible));
